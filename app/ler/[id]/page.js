...
export default function Reader({params}){
 ...
 {book.file_type==="pdf"?<iframe className="pdfReader" src={url} title={book.title}/>:<><div ref={epubRef} className="epubReader"/><div className="epubNav"><button className="goldButton" onClick={()=>window.dispatchEvent(new CustomEvent("epub-prev"))}>Anterior</button><button className="goldButton" onClick={()=>window.dispatchEvent(new CustomEvent("epub-next"))}>Próximo</button></div></>}
 </section>}
 </main>
}